...
class ToolpathGenerator {
  ...
  _genPocket(s, depth) {
    const PocketGenerator = require('./PocketGenerator');
  ...
  _genLine(s, depth) {
    ...
    const start = require('../utils/geometry').shapeStartPoint(s);
    const end   = require('../utils/geometry').shapeEndPoint(s);
    const feed  = s.feedRate || this.config.feedRateXY;
  ...
  _helicalPlunge(shape, targetZ) {
    ...
    const helixRPM  = this.config.spindleSpeed || 18000;
...
if (typeof module !== 'undefined') module.exports = ToolpathGenerator;
