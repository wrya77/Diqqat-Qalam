...
(function menuBar() {
  ...
  const ACTIONS = {
    ...
    'vp-offset':    () => ed()?.offsetPath(),
    'vp-outline':   () => ed()?.outlineStroke(),
    'vp-fillet':    () => ed()?.filletAllCorners(),
    'vp-chamfer':   () => ed()?.chamferCorners(),
    'vp-blend':     () => ed()?.blendShapes(),
    'vp-polar':     () => ed()?.polarArray(),
    'il-group':     () => ed()?.groupSelected(),
    'il-ungroup':   () => ed()?.ungroupSelected(),
    'il-same':      () => ed()?.selectSameFromMenu(),
    'il-shear':     () => ed()?.shearSelected(),
    'il-typepath':  () => ed()?.typeOnPath(),
    'il-vtype':     () => ed()?.verticalType(),
    'il-join':          () => ed()?.joinPaths(),
    'il-reverse':       () => ed()?.reversePathDir(),
    'il-addanchors':    () => ed()?.addAnchorPoints(),
    'il-cleanup':       () => ed()?.cleanupPath(),
    'il-transformeach': () => ed()?.transformEach(),
    ...
    'cv-distribute': () => ed()?.distributeOnPath(),
    'cv-kerf':       () => ed()?.toggleKerfPreview(),
    'cv-clampcheck': () => ed()?.checkClampCollisions(),
    'cv-time':       () => ed()?.estimateCutTime(),
    'cv-sheet':      () => ed()?.sheetUsageReport(),
    ...
    'bool-union':     () => ed()?.booleanOp('union'),
    'bool-subtract':  () => ed()?.booleanOp('difference'),
    'bool-intersect': () => ed()?.booleanOp('intersect'),
    'bool-xor':       () => ed()?.booleanOp('xor'),
    'bool-break':     () => ed()?.breakCompound(),
